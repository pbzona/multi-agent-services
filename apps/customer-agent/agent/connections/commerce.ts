...
const writeOperations = new Set(["setCartItemQuantity"]);
...
export default defineOpenAPIConnection({
  ...
  description:
    "Authenticated customer commerce API for products, the current cart, and the current customer's orders.",
  operations: {
    allow: [
      "listProducts",
      "getProduct",
  ...
  auth: (ctx) => {
    ...
    if (
      ...
      throw new Error("A verified customer demo session is required.");
    }

    const role = current.attributes.role;
    const workspaceId = current.attributes.workspaceId;
    if (role !== "customer" || typeof workspaceId !== "string") {
      throw new Error("The verified customer session context is invalid.");
    }
  ...
  approval: ({ toolName }) => {
    ...
    return {
      type: "denied",
      reason: `Commerce operation ${operation} is not allowed for customers.`,
    };
  },
});
